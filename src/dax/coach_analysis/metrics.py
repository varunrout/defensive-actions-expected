...
def add_suppression(df: pd.DataFrame) -> pd.DataFrame:
    out=df.copy()
    exp_shot=first_existing(out,['expected_shot_probability','pred_shot','shot_probability_oof','y_pred_proba','future_shot_pred'])
    obs_shot=first_existing(out,['observed_future_shot','future_shot','shot_within_10s','y_true'])
    exp_xg=first_existing(out,['expected_future_xg','pred_future_xg','future_xg_pred','y_pred'])
    obs_xg=first_existing(out,['observed_future_xg','future_xg','xg_within_10s','y_true_xg'])
...
def summary_table(df, group_cols):
    ...
    obs_shot=first_existing(df,['observed_future_shot','future_shot','shot_within_10s','y_true'])
    obs_xg=first_existing(df,['observed_future_xg','future_xg','xg_within_10s','y_true_xg'])
    exp_shot=first_existing(df,['expected_shot_probability','pred_shot','shot_probability_oof','y_pred_proba','future_shot_pred'])
    exp_xg=first_existing(df,['expected_future_xg','pred_future_xg','future_xg_pred','y_pred'])
    poss=first_existing(df,['possession_won','won_possession','defensive_action_won'])
    agg={}
