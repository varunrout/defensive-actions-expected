...
def add_suppression(df: pd.DataFrame) -> pd.DataFrame:
    ...
    obs_shot=first_existing(out,['coach_observed_shot','observed_future_shot','future_shot','shot_within_10s','y_true'])
    exp_xg=first_existing(out,['coach_expected_xg_r4','expected_future_xg','pred_future_xg','future_xg_pred','y_pred'])
    obs_xg=first_existing(out,['coach_observed_xg','observed_future_xg','future_xg','xg_within_10s','y_true_xg'])
...
def summary_table(df, group_cols):
    ...
    obs_shot=first_existing(df,['coach_observed_shot','observed_future_shot','future_shot','shot_within_10s','y_true'])
    obs_xg=first_existing(df,['coach_observed_xg','observed_future_xg','future_xg','xg_within_10s','y_true_xg'])
