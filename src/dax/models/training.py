...
def select_variant_rows(df: pd.DataFrame, contract: FeatureContract) -> VariantData:
    ...
    eligibility = {
        "rule": "has_360 is true and role-dependent 360 features are present",
        "requires_360": True,
        ...
        "visibility_quality_distribution": selected.get("visibility_quality", pd.Series(["unknown"] * len(selected))).value_counts(dropna=False).to_dict(),
    }
    return VariantData(
        selected,
        rows_excluded=int(len(df) - len(selected)),
...
def train_variant(
    ...
    validate_fold_support(variant_df, folds, contract)
