...
def processed_event_tables(df: pd.DataFrame) -> dict[str,pd.DataFrame]:
    ...
    out["event_counts_by_type"]=df["type"].value_counts(dropna=False).rename_axis("type").reset_index(name="rows")
    ...
    out["target_by_type"]=df.groupby("type",dropna=False).agg(rows=("match_id","size"),future_shot_rate=("target_future_shot_10s","mean"),future_xg_mean=("target_future_xg_10s","mean")).reset_index()
