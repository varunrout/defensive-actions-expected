...
from .plot_style import CLUSTER_COLOURS, DEFAULT_THEME, add_note
...
def _title(ax: plt.Axes, title: str, subtitle: str | None = None) -> None:
    ...
    if subtitle:
        ax.text(0, 1.02, subtitle, transform=ax.transAxes, fontsize=DEFAULT_THEME.subtitle_size, color="#555555")


def _direction_note(ax: plt.Axes) -> None:
    ax.annotate("Attacking direction", xy=(0.82, -0.08), xytext=(0.58, -0.08), xycoords="axes fraction", arrowprops={"arrowstyle": "->", "color": "#555555"}, color="#555555", fontsize=8)
...
def plot_pitch_density(df: pd.DataFrame, output_path: str | Path, *, title: str, bins: tuple[int, int] | None = None, x_col: str = "action_x", y_col: str = "action_y", config: dict[str, Any] | None = None, dpi: int | None = None, subtitle: str | None = None) -> plt.Figure:
    ...
    if {x_col, y_col}.issubset(data.columns) and not data.empty:
        ...
        fig.colorbar(pcm, ax=ax, label="Actions", shrink=0.82)
    _title(ax, title, subtitle or "Style distribution, not a performance map")
    _direction_note(ax)
    add_note(ax, f"Denominator: {len(data):,} actions. Bins: {use_bins[0]}x{use_bins[1]}; not tactical ground truth.")
    return _save(fig, output_path, dpi or int(cfg.get("chart_dpi", DEFAULT_THEME.dpi)))
...
def plot_pitch_rate_map(df: pd.DataFrame, output_path: str | Path, *, value_col: str, title: str, bins: tuple[int, int] | None = None, min_bin_actions: int = 20, x_col: str = "action_x", y_col: str = "action_y", config: dict[str, Any] | None = None, dpi: int | None = None) -> plt.Figure:
    ...
    if {x_col, y_col, value_col}.issubset(data.columns) and not data.empty:
        ...
        fig.colorbar(pcm, ax=ax, label=value_col.replace("_", " "), shrink=0.82)
    _title(ax, title, "Low-sample bins are masked")
    _direction_note(ax)
    add_note(ax, f"Cells with fewer than {min_bin_actions} actions are masked. Denominator: actions in each bin.")
    return _save(fig, output_path, dpi or int(cfg.get("chart_dpi", DEFAULT_THEME.dpi)))
...
def plot_cluster_population_difference(cluster_df: pd.DataFrame, population_df: pd.DataFrame, output_path: str | Path, *, title: str, config: dict[str, Any] | None = None) -> plt.Figure:
    ...
    if not cluster_df.empty and not population_df.empty:
        ...
        pcm = pitch.heatmap(c, ax=ax, cmap="RdBu_r", edgecolors="#f7f7f7")
        fig.colorbar(pcm, ax=ax, label="Cluster density minus population density", shrink=0.82)
    _title(ax, title, "Relative spatial style profile, not a performance map")
    _direction_note(ax)
