...
pipeline (Correlation Atlas V1/V2/V3, VIF Analysis) into reports/eda_xg/, so
the xG portal is a complete, self-contained arm -- not missing the pieces
that don't happen to change with the target.

These are NOT recomputed differently for xG: correlation (feature vs
feature) and VIF (a feature's own multicollinearity against every other
feature) never reference a target column at all -- the numbers are
...
and generate_vif_reports.py directly, reading the same already-computed
JSON (no re-analysis), and injects one small banner into each explaining
why the content matches the binary-target page exactly.
...
<p>Correlation (feature vs feature) and VIF (a feature's own multicollinearity against every other feature)
never reference a target column at all -- this page's numbers are <b>identical</b> to
