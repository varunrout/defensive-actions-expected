...
> **Superseded (Prompt 67):** this portal was built against `target_xt_delta` (v1), whose `xt_after`
> term was `xT(action_x, action_y)` — the defensive action's own recorded location. Prompt 66 found
> that definition wrong (for a Clearance it scores where the ball *was* when cleared, not where it
> ended up) and replaced it with a possession-outcome definition. The consequences are not marginal:
> Clearance moves from 8th of 8 event types (-0.0325) to 1st of 8 (+0.0247), and this document's
> single most important recorded caveat — construction coupling between location-derived features and
> `xt_after` — is roughly 74% weaker under the correction. See
> [`../xt_target/MASTER_FINDINGS.md`](../xt_target/MASTER_FINDINGS.md) and the rebuilt portal at
> [`../xt_target/INDEX.html`](../xt_target/INDEX.html). **This document and every report in this
> directory are kept exactly as-is, not deleted or rewritten** — they remain the accurate record of
> what Prompt 65 actually measured against v1 at the time.
