...
## 9. Rung 1 -- quadratic terms for U-shaped features (`v1b_quadratic`)

EDA (`active_numerical_target_atlas.json`, cross-checked in section 6) already
...
### 9.1 CV and held-out test results
...
### 9.2 Is the CV lift real? (`significance_v1_vs_v1b_quadratic.json`)
...
threshold. Wilcoxon: p=0.0625, the same n=5 exact-test floor seen in section
...
vs +0.0142 / +0.0182 in section 8.1) -- and it does **not** show up on the
...
-- confirming section 6's flagged mismatch (the *linear* v2 coefficient for
...
### 9.3 Player-disjoint re-check (`player_disjoint_v1b_quadratic.json`)
...
because v1 cleared it. Same protocol as section 8.3: 5-fold `GroupKFold` on
