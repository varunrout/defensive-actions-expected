...
> **Status update (Prompt 46):** `v1_unweighted` (this document's pick) has been superseded as
> the active-binary leg's standing reference model, first by `v1c_systematic_interactions`
> (Prompt 43), now by `v1e_gradient_boosting_calibrated` — see the promotion decision in
> `reports/eda/ACTIVE_BINARY_MODELLING_CLOSEOUT.md` section 8. This document remains the accurate
> historical record of the Rung 0 comparison and is not rewritten.
