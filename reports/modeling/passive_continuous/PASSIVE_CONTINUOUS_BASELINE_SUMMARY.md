...
*Split note (prompt 62): Rung 1 (`d1b_quadratic`, prompt 61) was originally appended directly below
this section, on the stated basis that the split into a separate ladder document would happen "once a
second rung is built." That threshold is now crossed (Rung 2, `d1d_random_forest`, prompt 62) --
Rungs 1 and 2 have moved to
[`PASSIVE_CONTINUOUS_MODEL_LADDER.md`](PASSIVE_CONTINUOUS_MODEL_LADDER.md), mirroring the
active-continuous leg's own split point (2 rungs beyond Rung 0, prompt 56). This document is
Rung-0-only from here on.*
