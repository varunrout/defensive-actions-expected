...
The canonical split is computed once by `scripts/compute_canonical_split.py` and frozen at `outputs/models/splits/match_assignment.json` (`{match_id: "test" | "fold0".."fold4"}`, all 115 matches, checked into git as a versioned artifact -- see the `.gitignore` exception carved out for this one file). It is built as:
