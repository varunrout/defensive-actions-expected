...
NUMERIC_FEATURES = [
    ...
    "visible_attacker_mean_distance",
    "visible_defender_mean_distance",
...
def _bucket_summary(df: pd.DataFrame, feature: str, target: str = "target_future_shot_10s") -> None:
...
def main() -> None:
    ...
    print(f"Shot rate: {df['target_future_shot_10s'].mean() * 100:.2f}%")
    print("\nAction families:\n" + df["action_family"].value_counts().to_string())
    print("\nPosition groups:\n" + df["position_group"].value_counts().to_string())

    print("\nTop phase shot rates:")
    phase_stats = df.groupby("phase_label", dropna=False)["target_future_shot_10s"].agg(["size", "mean"]).sort_values("mean", ascending=False)
    print(phase_stats.to_string())

    print("\nTop player-role shot rates (min 100 actions):")
    role_stats = df.groupby("position_group")["target_future_shot_10s"].agg(["size", "mean"])
    ...
    for feature in NUMERIC_FEATURES:
        if feature in df.columns:
            ...
            if valid.nunique() > 1 and valid.shape[0] > 10:
                corr[feature] = valid.corr(df.loc[valid.index, "target_future_shot_10s"])
