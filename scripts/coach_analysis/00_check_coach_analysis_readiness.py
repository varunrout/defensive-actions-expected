...
from dax.coach_analysis.loaders import CoachAnalysisInputError, input_status_from_paths, oof_coverage_for_variant, require_table, select_required_two_part, select_required_variant, validate_schema, validate_unique_predictions
from dax.coach_analysis.reporting import markdown_table, write_json, write_markdown_report
from dax.coach_analysis.timeline import normalise_processed_events, validate_processed_timeline
...
def main(argv: list[str] | None = None) -> int:
    ...
    status = input_status_from_paths({"actions": args.actions_input, "processed_events": args.processed_events_input, "classification_oof": args.classification_oof, "regression_oof": args.regression_oof, "two_part_oof": args.two_part_oof}, paths.root)
    actions = _read(args.actions_input, paths.root, errors)
    processed_events = normalise_processed_events(_read(args.processed_events_input, paths.root, errors))
    ...
    coverages = {
        "classification_primary": oof_coverage_for_variant(actions, b7, args.classification_variant),
        "classification_sensitivity": oof_coverage_for_variant(actions, b6, args.classification_sensitivity),
        "regression_primary": oof_coverage_for_variant(actions, r4, args.regression_variant),
        "regression_sensitivity": oof_coverage_for_variant(actions, r6, args.regression_sensitivity),
        "two_part": oof_coverage_for_variant(actions, tp, args.two_part_classification_variant),
    }
    for name, cov in coverages.items():
        if cov.get("native_coverage_rate") is not None and cov["native_coverage_rate"] < 1.0:
            errors.append(f"Prediction coverage below eligible population for {name}: {cov['native_coverage_rate']:.3f}")
