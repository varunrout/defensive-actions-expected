"""Train DAx baseline logistic variants (V0-V8)."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train baseline logistic models for DAx")
    parser.add_argument(
        "--input",
        type=str,
    ...
    parser.add_argument(
        "--validation-dir",
        type=str,
        default=str(REPO_ROOT / "outputs" / "validation" / "baseline"),
        help="Directory to save validation artifacts.",
    )
    parser.add_argument(
        "--oof-dir",
        type=str,
        default=str(REPO_ROOT / "outputs" / "oof" / "baseline"),
        help="Directory to save out-of-fold prediction artifacts.",
    )
    parser.add_argument("--n-splits", type=int, default=5)
    parser.add_argument("--max-rows", type=int, default=None, help="Optional quick-run row cap")
...
def main() -> None:
    ...
    print(f"Target rate: {df[TARGET_COL].mean() * 100:.3f}%")
    ...
    for spec in default_variant_specs():
        ...
        cv = grouped_cv_scores(x=x, y=y, groups=groups, pipeline=pipe, n_splits=args.n_splits)
        pipe.fit(x, y)

        coef_df = coefficient_table(pipe)
        coef_path = validation_dir / f"baseline_{resolved.name}_coefficients.csv"
        coef_df.to_csv(coef_path, index=False)

        model_path = models_dir / f"logistic_{resolved.name}.joblib"
        payload = {
        ...
        fold_path = validation_dir / f"baseline_{resolved.name}_fold_metrics.csv"
    ...
    summary_path = validation_dir / "baseline_model_metrics.json"
