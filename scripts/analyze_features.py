...
from dax.analysis.plotting import bar_chart, histogram, labelled_heatmap
...
def main() -> int:
    ...
    plot_pitch_scatter(df, spatial_dir / "all_actions_scatter.png", title="All defensive-action locations", config=config, dpi=dpi)
    plot_pitch_density(df, spatial_dir / "all_actions_density.png", title="All defensive-action density", config=config, dpi=dpi)
    if "action_won_possession" in df.columns:
        plot_pitch_rate_map(df, spatial_dir / "possession_win_rate_map.png", value_col="action_won_possession", title="Possession-win rate by pitch location", min_bin_actions=int(config.get("minimum_spatial_bin_actions", 20)), config=config, dpi=dpi)
    plot_pitch_rate_map(df, spatial_dir / "future_shot_rate_map.png", value_col="target_future_shot_10s", title="Future-shot rate by pitch location", min_bin_actions=int(config.get("minimum_spatial_bin_actions", 20)), config=config, dpi=dpi)
    plot_pitch_rate_map(df, spatial_dir / "future_xg_map.png", value_col="target_future_xg_10s", title="Future-xG mean by pitch location", min_bin_actions=int(config.get("minimum_spatial_bin_actions", 20)), config=config, dpi=dpi)
    if "is_defensive_box_action" in df.columns:
        plot_pitch_density(df[df["is_defensive_box_action"]], spatial_dir / "defensive_box_actions.png", title="Defensive-box action density", config=config, dpi=dpi)
    for family, family_df in df.groupby("action_family"):
        plot_pitch_density(family_df, spatial_dir / f"{family}_density.png", title=f"Action-family density: {family}", config=config, dpi=dpi)
    for phase, phase_df in df.groupby("phase_label"):
        safe_phase = str(phase).replace("/", "_").replace(" ", "_")
        plot_pitch_density(phase_df, spatial_dir / f"phase_{safe_phase}_density.png", title=f"Phase density: {phase}", config=config, dpi=dpi)
